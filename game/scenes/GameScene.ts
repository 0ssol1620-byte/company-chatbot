...
export class GameScene extends Phaser.Scene {
  ...
  // Multiplayer (BroadcastChannel-based tab sync)
  private localMultiplayer: LocalMultiplayer | null = null;
  ...
  create(): void {
        ...
        if (this.editorObjectMode) {
          ...
          if (pointer.rightButtonDown()) {
            ...
            if (objIndex >= 0) {
              ...
              this.renderObjects();
              this.localMultiplayer?.emit("map:object-remove", { objectId: removed.id });
            }
          } else {
            ...
            if (canPlaceObject(this.selectedObjectType, tileX, tileY, this.mapObjects, this.wallsData, this.selectedObjectDirection)) {
              ...
              this.renderObjects();
              this.localMultiplayer?.emit("map:object-add", { object: obj });
            }
          }
          return;
        }
    ...
    // Listen for multiplayer instance from React — may arrive before or after player spawn
    const handleMultiplayerReady = (data: {
    ...
    EventBus.emit("request-multiplayer");
  }
  ...
  private handleEditorClick(pointer: Phaser.Input.Pointer): void {
    ...
    this.localMultiplayer?.emit("map:tiles-update", { layer: layerName, row: tileY, col: tileX, tileId });
  ...
  private setupSocketListeners(): void {
    const lm = this.localMultiplayer;
    if (!lm) return;

    // Remote player sync (cross-tab via BroadcastChannel)
    lm.on("players:state", (data: unknown) => {
      const { players } = data as { players: RemotePlayerData[] };
      for (const p of players) this.addRemotePlayer(p);
    });

    lm.on("player:joined", (data: unknown) => {
      this.addRemotePlayer(data as RemotePlayerData);
    });

    lm.on("player:moved", (data: unknown) => {
      const d = data as { id: string; x: number; y: number; direction: string; animation: string };
      const remote = this.remotePlayers.get(d.id);
      if (remote) remote.updatePosition(d.x, d.y, d.direction, d.animation);
    });

    lm.on("player:left", (data: unknown) => {
    ...
    // Map editing cross-tab sync
    lm.on("map:object-added", (data: unknown) => {
      const { object } = data as { object: MapObject };
      this.mapObjects.push(object);
      this.renderObjects();
    });

    lm.on("map:object-removed", (data: unknown) => {
      const { objectId } = data as { objectId: string };
      this.mapObjects = this.mapObjects.filter(o => o.id !== objectId);
      this.renderObjects();
    });

    lm.on("map:tiles-updated", (data: unknown) => {
      const d = data as { layer: string; row: number; col: number; tileId: number };
      if (this.tiledMode) return;
      if (d.layer === "floor" && this.floorData[d.row]) {
        this.floorData[d.row][d.col] = d.tileId;
      } else if (d.layer === "walls" && this.wallsData[d.row]) {
        this.wallsData[d.row][d.col] = d.tileId;
      }
      this.createTilemap();
    });

    lm.on("npc:stop-moving", (data: unknown) => {
      const { npcId } = data as { npcId: string };
    ...
    lm.on("npc:position-sync", (data: unknown) => {
      const d = data as { npcId: string; x: number; y: number; direction: string };
      const npc = this.npcSprites.find(n => n.id === d.npcId);
      if (!npc) return;
      npc.pixelX = d.x;
      npc.pixelY = d.y;
      npc.direction = DIR_NAME_MAP[d.direction] ?? DIR_DOWN;
      npc.sprite.setPosition(d.x, d.y);
      npc.nameLabel.setPosition(d.x, d.y - 44);
      if (npc.sprite instanceof Phaser.GameObjects.Sprite) {
        const walkKey = `npc-${npc.id}-walk-${d.direction}`;
  ...
  private joinMultiplayer(x: number, y: number): void {
    if (!this.localMultiplayer || !this.characterId) return;
    this.localMultiplayer.emit("player:join", {
      characterId: this.characterId,
      characterName: this.characterName,
      appearance: this.appearance,
      mapId: this.channelId || "default",
      x,
      y,
    });
  }
    ...
    if (this.player) {
      ...
      for (const npc of this.npcSprites) {
        if (npc.moveState === "waiting" && !this.dialogOpen) {
          ...
          if (npc.waitTimer >= npc.waitDurationMs) {
            ...
            this.localMultiplayer?.emit("npc:return-home", { channelId: this.channelId, npcId: npc.id });
          }
        }
      }
      ...
      for (const npc of this.npcSprites) {
        ...
        if (result === "arrived") {
          ...
          this.localMultiplayer?.emit("npc:arrived", {
      ...
      if (this.npcPositionSyncTimer >= 200) {
        ...
        for (const npc of this.npcSprites) {
          if (npc.moveState === "moving-to-player" || npc.moveState === "returning") {
            this.localMultiplayer?.emit("npc:position-update", {
              channelId: this.channelId,
              npcId: npc.id,
              x: npc.pixelX,
              y: npc.pixelY,
